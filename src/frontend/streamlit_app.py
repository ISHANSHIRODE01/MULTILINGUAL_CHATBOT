...
with col1:
    ...
    st.markdown("*Experience seamless voice conversations in German.*")
    
    # Chat Container
    st.markdown('<div class="glass-container" style="min-height: 400px;">', unsafe_allow_html=True)
    
    if not st.session_state.messages:
        st.info("👋 Welcome! Upload an audio file to start the conversation.")
    ...
    # File Uploader
    uploaded = st.file_uploader("Or Upload Audio File", type=SUPPORTED_FORMATS)
            ...
            if st.button("🚀 Process Audio", type="primary"):
                with st.spinner("🔄 Transcribing & Translating..."):
                    try:
                        ...
                        input_audio.seek(0)
                        files = {"file": (input_audio.name, input_audio.getvalue(), input_audio.type)}
                        data = {"target_lang": target_lang}
                        
                        resp = requests.post(f"{API_BASE}/chat_audio", files=files, data=data, timeout=45)
                        ...
                        if resp.status_code == 200:
                            ...
                            if data.get("success"):
                                ...
                                st.session_state.messages.append({
                                    "role": "user",
                                    "content": res.get("user_text"),
                                    "lang": res.get("detected_lang")
                                })
...
with col2:
    ...
    - **Neural TTS**: Natural German voice
